/*
 * SCR-021 / Daily Sales / Default — Figma 134:11150
 * 정적 플레이스홀더. 조회·집계는 이후 sales adapter 소유.
 */
import AdminTopHeader from "../../components/admin/AdminTopHeader.jsx";

const HOURLY_BARS = [40, 30, 45, 60, 50, 35, 25, 20, 15, 10, 5, 5];
const HOURLY_TICKS = ["10시", "12시", "14시", "16시", "18시", "20시", "21시"];
const PEAK_INDEX = 3;
      ...
      <div className="sales-share-card__rows">
        {rows.map(([label, percent, fill, isPrimary]) => (
          <div className="sales-share-card__row" key={label}>
            ...
            <i>
              <em className={isPrimary ? "is-primary" : ""} style={{ width: `${fill}%` }} />
            </i>
          </div>
        ))}
      </div>
    ...
    <section className="sales-daily" data-figma-node="134:11150">
      ...
      <div className="sales-daily__middle">
        ...
        <div className="sales-daily__right">
          <ShareCard title="결제수단별 매출" rows={paymentShare} />
          <ShareCard title="주문 유형별 매출" rows={orderShare} />
      ...
      <section className="sales-table">
        ...
        <div className="sales-table__grid sales-table__grid--hourly">
          ...
          {HOURLY_ROWS.map(([slot, count, sales, avg]) => (
            <div className="sales-table__row" key={slot}>
              <span>{slot}</span>
              <span>{count}</span>
              <span>{sales}</span>
              <span>{avg}</span>
